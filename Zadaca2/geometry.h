...
// type specialization depending on float or int and dimension size 
...
// dot product implementation (skalarni produt)
...
// vector subtraction rhs
template<size_t DIM,typename T>vec<DIM,T> operator-(vec<DIM,T> lhs, const vec<DIM,T>& rhs) {
    for (size_t i=DIM; i--; lhs[i]-=rhs[i]);
    return lhs;
}
// product by components implementation
